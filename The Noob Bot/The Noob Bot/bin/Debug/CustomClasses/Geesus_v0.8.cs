    ...
    public Blood()
    {
        ...
        while (Main.loop)
        {
            ...
            if (!ObjectManager.Me.IsMounted)
            {
                ...
                if (!Fight.InFight &&
                    Tailoring.KnownSpell &&
                    ItemsManager.GetItemCountByIdLUA(53010) > 4 && fail < 10 && look.IsReady)
                {
                    boltcount = ItemsManager.GetItemCountByIdLUA(53643);
                    Logging.WriteFight("Create Bolt");
                    Fight.StopFight();
                    MovementManager.StopMove();
                    nManager.Wow.Helpers.Keybindings.PressBarAndSlotKey("1;10");
                    Thread.Sleep(300);
                    Thread.Sleep(300);
                    ...
                    Thread.Sleep(2000);
                    if (boltcount == ItemsManager.GetItemCountByIdLUA(53643))
                    {
                        fail = fail + 1;
                        Logging.WriteFight("Create Bolt Failed - Macro ?");
                    }
                    else fail = 0;
                }

                if (!Fight.InFight &&
                    Tailoring.KnownSpell && look.IsReady &&
                    ItemsManager.GetItemCountByIdLUA(53643) > 14 &&
                    ItemsManager.GetItemCountByIdLUA(52555) > 14)
                {
                    Logging.WriteFight("Create A Bag");
                    Fight.StopFight();
                    MovementManager.StopMove();
                    nManager.Wow.Helpers.Keybindings.PressBarAndSlotKey("1;11");
                    while (ObjectManager.Me.IsCast)
                    {
                        Thread.Sleep(100);
                        Thread.Sleep(100);
