    ...
    public sealed partial class BlackMagic
    {
        ...
        public bool Open(int ProcessId)
        {
            ...
            if (m_bProcessOpen)
            {
                ...
                m_Modules = System.Diagnostics.Process.GetProcessById(m_ProcessId).Modules;
        ...
        {
            foreach (ProcessModule pMod in m_Modules)
                if ((uint) pMod.BaseAddress <= dwAddress &&
                    ((uint) pMod.BaseAddress + pMod.ModuleMemorySize) >= dwAddress)
                    return pMod;
