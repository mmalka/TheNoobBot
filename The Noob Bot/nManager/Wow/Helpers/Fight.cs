...
namespace nManager.Wow.Helpers
{
    public class Fight
    {
        ...
        public static ulong StartFight(ulong guid = 0, bool inBg = false)
        {
            ...
            try
            {
                if (true)//!ObjectManager.ObjectManager.Me.IsMounted)
                {
                    if (guid == 0)
                    {
                        targetNpc =
                    ...
                    //targetNpc = new WoWUnit(targetNpc.GetBaseAddress);

                    _fightLoop = true;

                    if (!ObjectManager.ObjectManager.Me.IsCast)
                        Interact.InteractGameObjectBeta23(targetNpc.GetBaseAddress);
                    Thread.Sleep(100);
