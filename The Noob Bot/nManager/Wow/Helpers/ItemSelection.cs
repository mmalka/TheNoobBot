            ...
            if (itemValueForMe != 0) // Item interesting
            {
                ...
                if (questRewardItem.ItemEquipLoc == "INVTYPE_FINGER" || questRewardItem.ItemEquipLoc == "INVTYPE_TRINKET") // Then check the second one too
                {
                    ...
                    if (equipedItem2.ItemRarity < 7 && // Just consider equiped Heirloom > all
                        ((questRewardItem.ItemRarity > equipedItem2.ItemRarity &&
                          questRewardItem.ItemLevel > (equipedItem2.ItemLevel - (5*equipedItem2.ItemLevel/100)*(questRewardItem.ItemRarity - equipedItem2.ItemRarity)))
                         || (questRewardItem.ItemRarity == equipedItem2.ItemRarity && questRewardItem.ItemLevel > equipedItem2.ItemLevel)
                         ||
                         (questRewardItem.ItemRarity < equipedItem2.ItemRarity &&
                          questRewardItem.ItemLevel > (equipedItem2.ItemLevel + (5*equipedItem2.ItemLevel/100)*(equipedItem2.ItemRarity - questRewardItem.ItemRarity)))))
                    {
                        itemValueForMe = -1;
