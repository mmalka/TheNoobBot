...
if ((unit.IsValid && !nManagerSetting.IsBlackListedZone(unit.Position) && !nManagerSetting.IsBlackListed(unit.Guid)) || (node.IsValid && !nManagerSetting.IsBlackListedZone(node.Position) && !nManagerSetting.IsBlackListed(node.Guid)))
{
	...
	WoWGameObject fruit = ObjectManager.GetNearestWoWGameObject(ObjectManager.GetWoWGameObjectById(Fruits));
	
	if(fruit.IsValid)
	{
		Logging.Write("Fruit trouver");
		MovementManager.Face(fruit);
		Interact.InteractWith(fruit.GetBaseAddress);
		nManagerSetting.AddBlackList(fruit.Guid, 30*1000);
	}
